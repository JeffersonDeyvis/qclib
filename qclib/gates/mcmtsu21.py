...
class MTMCSU21(Gate):
    ...
    def rzryrz(w):
        ...
        B = np.array([np.angle(w[0][0]), np.angle(w[0][1])])
        x = A_inv @ B

        alpha = x[0]
        beta = x[1]
        norm = np.linalg.norm(w[0][0])
        theta = 2 * np.arccos(norm)

        return [alpha, beta, theta]

    @staticmethod
    def r(axis, theta):
        """Returns the rotation matrix Rj(theta)"""
        if axis == 'z':
    ...
    @staticmethod
    def abc(operator, unitary):

        angles = MTMCSU21.rzryrz(unitary)
        if operator == 'A':
            a = MTMCSU21.r('z', angles[0]) @ MTMCSU21.r('y', angles[2] / 2)
            return Operator(a)

        elif operator == 'B':
            b = (MTMCSU21.r('y', -angles[2] / 2) @
                 MTMCSU21.r('z', -(angles[0] + angles[1]) / 2))
            return Operator(b)

        elif operator == 'C':
            c = MTMCSU21.r('z', (angles[1] - angles[0]) / 2)
            return Operator(c)

    def mcmtsu2(self):
        circ = QuantumCircuit(self.controls, self.target)
        for index, unitary in enumerate(self.unitaries):
            op_a = MTMCSU21.abc('A', unitary)
            circ.append(op_a, [index + len(self.controls)])

        # colocar mcx

        for index, unitary in enumerate(self.unitaries):
            op_b = MTMCSU21.abc('B', unitary)
            circ.append(op_b, [index + len(self.controls)])

        # colocar mcx

        for index, unitary in enumerate(self.unitaries):
            op_c = MTMCSU21.abc('C', unitary)
            circ.append(op_c, [index + len(self.controls)])
